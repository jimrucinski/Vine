...
//		add_action('profile_update', array(&$this, 'avatar_upload'));												// Perform avatar image upload.
	...
	function manage_avatar_cache()
	{
		...
		if(isset($_GET['user_id'])) {
			$msg = __('Employee Photo', 'avatars') . ' ' . (empty($_GET['avatar']) ? __('removed', 'avatars') : __('updated', 'avatars')) . '.';
		...
		<div class="wrap">
			<span id='icon-users' class='icon32'></span><h2><?php _e('Employee Photo Settings', 'avatars'); ?></h2>
			
			<h3><?php _e('Employee Photos', 'avatars'); ?></h3>
			<?php
		...
		if ($this->networked) {
			if( is_site_admin() ) {
				if(version_compare($wp_version, '3.1', '<')) {
					add_submenu_page('wpmu-admin.php', __('Employee Photos', 'avatars'), __('Employee Photos', 'avatars'), 'unfiltered_html', 'add_local_avatar', array(&$this, 'manage_avatar_cache'));
				}
			}
		}
		else {
			if(current_user_can('manage_options')) {
				add_users_page(__('Employee Photos', 'avatars'), __('Employee Photos', 'avatars'), 1, basename(__FILE__), array(&$this, 'manage_avatar_cache') );
			}
		}
	}
	...
	{
		add_submenu_page('settings.php', __('Employee Photos', 'avatars'), __('Employee Photos', 'avatars'), 'manage_options', 'add_local_avatar', array(&$this, 'manage_avatar_cache'));
	}
	...
	{ ?>
		...
		<?php
			...
			if($this->avatar_options['upload_allowed'] == 'on' || current_user_can('edit_users')) {
				...
				else _e('Photo uploads not allowed (Administrator may set on <i>Users &raquo; Employee Photos</i> page).', 'avatars');
			}
		echo '</span>';
	}
