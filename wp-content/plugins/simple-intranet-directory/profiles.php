...
define('SCALED_SIZE', '100');															// Default size to scale uploads to.
...
class add_local_avatars {
	...
	function __construct()
	{
		...
		if(file_exists(ABSPATH . '/wp-content/plugins/profile/avatars-widget.php')) {
			include(ABSPATH . '/wp-content/plugins/profile/avatars-widget.php');
...
//		add_action('profile_update', array(&$this, 'avatar_upload'));												// Perform avatar image upload.
	...
	function avatars_initialise()
	{
		...
		if(is_admin()) {
			wp_enqueue_style('avatars', plugins_url('/profile/avatars-admin.css'), 'css');
		}
		else {
			wp_enqueue_style('avatars', plugins_url('/profile/avatars.css'), 'css');
		}
		...
		$test = WPLANG;
		if(!empty($test)) {
			load_plugin_textdomain('avatars', 'wp-content/plugins/profile');
		}
	}
	...
	function manage_avatar_cache()
	{
		...
		<div class="wrap">
			<span id='icon-users' class='icon32'></span><h2><?php _e('User Photo Settings', 'avatars'); ?></h2>
			
			<h3><?php _e('User Photos', 'avatars'); ?></h3>
			<?php
				...
				if(!get_option('show_avatars')) {
					...
					<table class='widefat'>
						<thead>
							<tr>
								...
								<th style="text-align: center;"><?php _e('Photo', 'avatars'); ?></th>
					...
					<p><?php
						if(!$all) echo __("Not showing photos for commenters.", 'avatars'), " <a href='" . $this->requestURI . "&amp;act=all'>", __('Click here', 'avatars'), '</a> ', __("to show commenter avatars.", 'avatars');
						else echo __("Showing avatars for commenters.", 'avatars'), " <a href='" . $this->requestURI . "'>", __('Click here', 'avatars'), '</a> ', __("to hide commenter photos.", 'avatars');
					?></p>
						...
						if($all) {
							...
							if($total > $per_page) {
								$paging_text = paginate_links(array(
									...
									'current' => $com_page,
									'base' => 'users.php?page=profiles.php&amp;act=all&amp;%_%',
									'format' => 'comspage=%#%'
								));
							}

							if($coms) { ?>
								<h3><?php _e('Commenter Photos', 'avatars'); ?></h3>
								...
								<table class='widefat'>
									<thead>
										<tr><th><?php _e('Name', 'avatars'); ?></th><th><?php _e('e-Mail', 'avatars'); ?></th><th style="text-align: center;"><?php _e('Comments', 'avatars'); ?></th><th style="text-align: center;"><?php _e('Photo', 'avatars'); ?></th></tr>
									</thead>
									<tbody> <?php
									$i = 0;
			...
			<h3><?php _e('Photo Options', 'avatars'); ?></h3>
			<form method="post" action="<?php echo $form_action; ?>">
				<table class='form-table'>
					...
					<tr>
						...
						<td><small><?php _e('Enable Photos.', 'avatars'); ?></small></td>
					</tr>
					<tr>
						<td><?php _e('Photo rating:', 'avatars'); ?><br /><small><?php _e('Repeated from <i>Settings  &raquo;  Discussion</i>', 'avatars'); ?></small></td>
						<td>
					...
					<?php if(class_exists('SimpleXMLElement')) { ?>
						<tr>
							<td><label for="idTwitter"><?php _e('Twitter Photo:', 'avatars'); ?></label></td>
					...
					<tr>
						<td><label for="idAvatarInPosts"><?php _e('Photos in posts:', 'avatars'); ?></label></td>
					...
					<tr>
						<?php if(file_exists(ABSPATH . '/wp-content/plugins/profile/avatars-widget.php')) { ?>
					...
					<tr>
						<td><label for="idWrapAvatar"><?php _e('Wrap Photos with URL:', 'avatars'); ?></label></td>
						<td><input type="checkbox" id="idWrapAvatar" name="url_wrap" <?php echo $this->avatar_options['url_wrap'] == 'on' ? 'checked' : ''; ?> /></td>
						<td><small><label for="idWrapAvatar"><?php _e("Wrap Photo with URL (from User's profile or Comment form data).", 'avatars'); ?></label></small></td>
					</tr>
					<tr>
				...
				<p class="submit"><input type="submit" name="submit" class="button-primary" value="<?php _e('Update Photo Options', 'avatars'); ?>" /></p>
			</form>
	...
	function avatar_footer()
	{
		if($this->avatar_options['credit'] != 'off') {
			printf("", ($this->avatar_options['credit'] == 'vis' ? '' : "style='display: none;'"));
		}
	}
		...
		if ($this->networked) {
			if( is_site_admin() ) {
				if(version_compare($wp_version, '3.1', '<')) {
					add_submenu_page('wpmu-admin.php', __('User Photos', 'avatars'), __('User Photos', 'avatars'), 'unfiltered_html', 'add_local_avatar', array(&$this, 'manage_avatar_cache'));
				}
			}
		}
		else {
			if(current_user_can('manage_options')) {
				add_users_page(__('User Photos', 'avatars'), __('User Photos', 'avatars'), 1, basename(__FILE__), array(&$this, 'manage_avatar_cache') );
			}
		}
	}
	...
	{
		add_submenu_page('settings.php', __('User Photos', 'avatars'), __('User Photos', 'avatars'), 'manage_options', 'add_local_avatar', array(&$this, 'manage_avatar_cache'));
	}
	...
	{ ?>
		...
		<?php
			...
			if($this->avatar_options['upload_allowed'] == 'on' || current_user_can('edit_users')) {
				...
				else _e('Photo uploads not allowed (Administrator may set on <i>Users &raquo; User Photos</i> page).', 'avatars');
			}
		echo '</span>';
	}
		...
		if(($uid == $current_user->ID || current_user_can('edit_users')) &&  is_numeric($_GET['uid'])) { ?>
			...
			<body>
				...
				<span style='width: 497px; padding:8px; float: right;'>
					<h3>Image Upload</h3>
					<?php printf("<p><small>%s</small></p>", __('Upload an image to use as an Photo.', 'avatars')); ?>
	...
	function avatar_upload($user_id)
	{
		...
		if(isset($_FILES['avatar_file']) && @$_FILES['avatar_file']['name']) {	// Something uploaded?
			...
			// Save the new local avatar for this user.
			if(empty($error)) update_user_meta($user_id, 'avatar', $path . $file);
		}

		// If there was an an error, record the text for display.
		if(!empty($error)) update_user_meta($user_id, 'avatar_error', $error);
...
function get_avatar($id_or_email, $size = '', $default = '', $post = false)
{
	...
	elseif(is_object($id_or_email)) {										// Comment object...
		if(!empty($id_or_email->user_id)) {									// Object has a user ID, commenter was registered & logged in...
			...
			switch($id_or_email->comment_type) {
				...
				case '':
					...
					$user = get_user_by('email',$email); // used to be get_userdatabylogin_email($email);
		...
		$user = get_user_by('email',$email); // used to be get_userdatabylogin_email($email);
		//$user = get_userdatabylogin_email($email);
		if($user) $id = $user->ID;											// Set ID if we can to check for local avatar.
	}
